...
void Decode_Using_DPCM (char *in_filename_Ptr) {
    ...
    char c;
    int row, col, width, height, max_gray_value, prediction_rule, pixel_count;
    struct PGM_Image pic_pgm;

    // Open file for reading
    FILE *DPCM_file_pointer =  fopen(in_filename_Ptr, "rb");
    if(DPCM_file_pointer == NULL) printf("Error opening lz file for reading");
    ...
    prediction_array[0][0] = geti(DPCM_file_pointer);

    // Top west row
    for(col = 1; col < width; col++) {
        prediction_array[0][col] = geti(DPCM_file_pointer);
        pic_pgm.image[0][col] = prediction_array[0][col] + pic_pgm.image[0][col-1];
    }

    // Second north row 
    for(col = 0; col < width; col++) {
        prediction_array[1][col] = geti(DPCM_file_pointer);
    ...
    // Last north column
    for(row = 2; row < height; row++) {
        prediction_array[row][width-1] = geti(DPCM_file_pointer);
